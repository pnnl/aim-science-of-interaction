 ...
 *
 */
public class OldNMRSampleStreamMonitor {
  ...
  public final static String[] STATUS = new String[] { "Compressive Analysis process has failed. Data is being passed as-is.", "All is well.", "Fault detected. This scan is outside of expected values due most likely to a failure of some type in the instrument.", "Point of diminishing returns was reached." };

  public final static int[] STATUS_SWAP = new int[] { 1, 0, 2, 3 };

  private final Set<String> knownCompounds = new HashSet<String>();

  private OldNMRReactorState state = new OldNMRReactorState();

  private final Map<String, NMRSample> mapSampleIDToSample = new HashMap<String, NMRSample>();

  public OldNMRSampleStreamMonitor() {
  ...
  public static String getStatus(final int code) {
    if (code >= 0 && code < OldNMRSampleStreamMonitor.STATUS.length) {
      return OldNMRSampleStreamMonitor.STATUS[code];
    }
   ...
   * @return
   */
  public OldNMRReactorState getNMRReactorState() {
    return state;
  }
  ...
  private void incorporateCompressiveAnalysisMessage(final NmrOpaStreamRecord msg) {
    ...
    //    currentSample.setHasCAValues(true);
  ...
  private void incorporateShyreMessage(final ShyreNmrMessage msg) {
    ...
    //    currentSample.setHasShyreValues(true);
  ...
  private void incorporateOPAMessage(final OpaNmrMessage msg) {
    ...
    //    currentSample.setHasOPAValues(true);
    ...
    for (int i = 0; i < COMPOUNDS.length; i++) {
      ...
      if (i < probabilities.size()) {
        ...
        m.setProbability(probabilities.get(i));
      }
    }
  ...
  private void rebuildNMRReactorState() {
    OldNMRReactorState stateFresh = new OldNMRReactorState();
    ...
    //      NMRSample sample = mapSampleIDToSample.get(sampleIds.get(i));
    //      if (sample.isHasCAValues()) {
    //        latestCA = i;
    //      }
    //      if (sample.isHasOPAValues()) {
    //        latestOPA = i;
    //      }
    //      if (sample.isHasShyreValues()) {
    //        latestShyre = i;
    //      }
    //    }

    System.out.println("Latest: " + sampleIds.get(latestCA) + ", " + sampleIds.get(latestOPA) + ", " + sampleIds.get(latestShyre));

    int currentId = Math.max(0, Math.min(latestCA, latestOPA)); //Math.min(latestOPA, latestShyre)));
    System.out.println("CurrentID: " + currentId + ", " + sampleIds.get(currentId) + ", " + sampleIds);
    stateFresh.setCurrent(mapSampleIDToSample.get(sampleIds.get(currentId)));

    for(int i = currentId - 1; i >= 0; i--) {
      stateFresh.getHistory().add(mapSampleIDToSample.get(sampleIds.get(i)));
    }

    //    Set<Entry<String, NMRSample>> entrySet = mapSampleIDToSample.entrySet();
    //    List<Entry<String, NMRSample>> entryList = new ArrayList<Entry<String, NMRSample>>();
    //    entryList.addAll(entrySet);
    //
    //    entryList.sort(new Comparator<Entry<String, NMRSample>>() {
    //      @Override
    //      // The map keys are the Run Number which is a number.
    //      // Sort descending. Assume larger Run Number should be first in the display.
    //      public int compare(final Entry<String, NMRSample> o1, final Entry<String, NMRSample> o2) {
    //        return Long.compare(Long.parseLong(o2.getKey()), Long.parseLong(o1.getKey()));
    //      }
    //    });
    //
    //    int entryCount = entryList.size();
    //    if (entryCount > 0) {
    //      List<NMRSample> history = new ArrayList<NMRSample>();
    //
    //      // Set the current to be the first entry in the list
    //      if (entryCount == 1) {
    //        stateFresh.setCurrent(entryList.get(0).getValue());
    //      } else {
    //        // Iterate over the remaining samples and put them in history
    //        int lastCompleteIndex = -1;
    //        int displayCount = Math.min(entryCount, NMRConfig.MAX_SAMPLES_DISPLAYED);
    //        for (int i = 0; i < entryCount; i++) {
    //
    //          NMRSample sample = entryList.get(i).getValue();
    //
    //          if (sample.isPredictedByOPA() && sample.isPredictedByShyre()) {
    //
    ...
    //            history.add(sample);
    //
    //            // Obey the limit for how many samples to show in the history
    ...
    //        }
    //        if (lastCompleteIndex != -1) {
    //          stateFresh.setCurrent(entryList.get(lastCompleteIndex - 1).getValue());
    //        }
    //      }
    //
    //      stateFresh.setHistory(history);
    //      System.out.println("NMR state refreshed ... samples: " + entryCount + " history: " + history.size());
    //    }

    state = stateFresh;
  }

  public NMRSample getSample(final String sampleID) {
